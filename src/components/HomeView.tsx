...
export function HomeView({ onOpenClassroom }: HomeViewProps) {
  ...
  const { loadFromStorage, sessionSummary } = useTokenStore();
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">AI Teaching Engine v3.1</p>
            </div>
          </div>
          ...
          <Card className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/30">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium">Resume your last lesson?</p>
                  <p className="text-xs text-muted-foreground">
                    You have an unsaved session from{' '}
                    {new Date(savedSession.saved_at).toLocaleString()}
                  </p>
                </div>
              </div>
              <Button
          ...
          {filteredClassrooms.length === 0 ? (
            <Card className="border-dashed">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="font-medium mb-1">No classrooms yet</h3>
                <p className="text-sm text-muted-foreground mb-4">
                  Create your first classroom to start learning with AI
                </p>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this classroom and all its data. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
