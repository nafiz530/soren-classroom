...
export function ClassroomCard({ classroom, onOpen, onDelete }: ClassroomCardProps) {
  const statusInfo = statusLabels[classroom.status] || statusLabels.active;
    ...
    <Card className="group relative overflow-hidden transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 border-border/60">
      <CardContent className="p-4 sm:p-5">
        ...
        {classroom.last_session_summary && (
          <p className="text-xs text-muted-foreground mt-3 line-clamp-2" dir="auto">
            {classroom.last_session_summary}
          </p>
        )}
        ...
        <div className="flex items-center gap-2 mt-4">
          <Button
            size="sm"
            className="flex-1 bg-gradient-to-r from-amber-500 to-rose-500 hover:from-amber-600 hover:to-rose-600"
            onClick={() => onOpen(classroom.id)}
          >
            <Play className="h-3.5 w-3.5 mr-1.5" />
            {classroom.status === 'active' ? 'চালু করুন / Continue' : 'শুরু করুন / Start'}
          </Button>
          <Button
            size="sm"
            variant="ghost"
