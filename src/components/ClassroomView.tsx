...
import { ArrowLeft, Settings, Volume2, VolumeX, Maximize2, Minimize2, BookOpen } from 'lucide-react';
...
const PHASE_LABELS: Record<FlowPhase, { label: string; labelBn: string; color: string }> = {
  ...
  loading: { label: 'Teacher is thinking...', labelBn: 'শিক্ষক ভাবছেন...', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300' },
  introducing: { label: 'Introducing', labelBn: 'ভূমিকা', color: 'bg-sky-100 text-sky-700 dark:bg-sky-900 dark:text-sky-300' },
  explaining: { label: 'Explaining', labelBn: 'ব্যাখ্যা', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300' },
  exampling: { label: 'Example', labelBn: 'উদাহরণ', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300' },
  quizzing: { label: 'Quiz', labelBn: 'কুইজ', color: 'bg-rose-100 text-rose-700 dark:bg-rose-900 dark:text-rose-300' },
  recapping: { label: 'Recap', labelBn: 'সারসংক্ষেপ', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300' },
  completed: { label: 'Completed', labelBn: 'সম্পন্ন', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300' },
...
export function ClassroomView({ classroom, onBack }: ClassroomViewProps) {
  ...
  const handleGenerateLesson = async (query: string) => {
    ...
    try {
      ...
      // Track topic learned
      incrementTopics(classroom.subject);
    ...
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      ...
      <header className="shrink-0 border-b border-border bg-card/80 backdrop-blur-sm px-3 py-2 flex items-center gap-2 z-30">
        ...
        <Badge variant="secondary" className={`text-[10px] shrink-0 ${phaseInfo.color}`}>
          {phaseInfo.labelBn !== phaseInfo.label ? `${phaseInfo.labelBn} · ` : ''}{phaseInfo.label}
        </Badge>
        ...
        <Select value={languageMode} onValueChange={(v) => setLanguageMode(v as LanguageMode)}>
          ...
          <SelectContent>
            <SelectItem value="both">বাং+EN</SelectItem>
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {/* Empty State — when no lesson is loaded */}
        {phase === 'idle' && boardBlocks.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
            <div className="text-center space-y-3 max-w-sm mx-auto px-4">
              <div className="text-5xl opacity-40">🎓</div>
              <h2 className="text-lg font-semibold text-muted-foreground/80">
                ক্লাসে স্বাগতম!
              </h2>
              <p className="text-sm text-muted-foreground/60">
                তোমার বিষয় বা প্রশ্ন লিখে ক্লাস শুরু করো
              </p>
              <p className="text-xs text-muted-foreground/40">
                Type your topic or question below to start the class
              </p>
            </div>
          </div>
        )}
        ...
        {phase === 'loading' && (
          <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
            <div className="text-center space-y-3 bg-card/90 backdrop-blur-sm rounded-2xl p-6 border border-border shadow-lg">
              <div className="flex justify-center">
                <div className="relative">
                  <BookOpen className="h-8 w-8 text-amber-500 animate-pulse" />
                </div>
              </div>
              <h3 className="text-sm font-semibold">অধ্যাপক পড়াচ্ছেন...</h3>
              <p className="text-xs text-muted-foreground">Teacher is preparing your lesson</p>
      ...
      <InputBar
        ...
        placeholder={
          phase === 'idle'
            ? 'বিষয় বা প্রশ্ন লিখুন... / Enter a topic or question...'
            : phase === 'completed'
            ? 'আরেকটি প্রশ্ন করুন... / Ask another question...'
            : phase === 'loading'
            ? 'শিক্ষক পড়াচ্ছেন... / Teacher is teaching...'
            : 'প্রশ্ন করুন... / Ask a follow-up...'
        }
      />
    </div>
  );
